...
## `GET /api/v1/apps/tbwatcher/items`

The Sell Advisor's **ItemKey → name bridge map**, served verbatim as JSON. The stash reads the player's save,
which stores each item only as a numeric `ItemKey`; this map turns those into Steam `market_hash_name`s. It is
extracted from the installed game's Unity Localization tables (`tbwatcher/tools/extract_items.py`) and served
here so a game update (new items) reaches clients **without a client rebuild** — the client fetches it at
startup, caches it to disk, and falls back to the copy embedded at build time on any failure.

Shape (`save::mapping::ItemMap`) — names resolve keys at lookup, so it is small (~40 KB), not a full key list:
```json
{
  "names":     { "110001": "Minor Ruby", "300001": "Long Sword", "310019": "Eternal Bow" },
  "gearIcons": { "300001": "/game/gear/sword/SWORD_300001.png" },
  "materials": { "110001": { "type": "Decoration", "grade": "Common", "icon": "/game/items/materials/Item_110001.png" } }
}
```
- `names` — id → English name: a **material** by its full key, a **gear** item by its base id
  (`base_id = key[0:2] + "00" + key[3:5]`); grade is the key's 3rd digit. The market hash is composed at
  lookup: gear → `"<name> (<grade>) <A|B|C>"` (the game reuses the trailing letter per line, so the client
  tries A/B/C plus the bare form and keeps whichever is listed); material → the plain name.
- `gearIcons` / `materials` — cosmetic only (stash-grid sprite, fx-class, rarity colour); a brand-new item
  with no entry shows a placeholder, never a wrong value.

Backend serves `database/data/tbwatcher/item_map.json` verbatim; 404 if absent (client uses its embedded map).
