...
use market::{MarketDataSource, MarketQuote, SteamClient, SteamMarketSource};
use save::SaveReader;
...
async fn watchlist_refresh(state: State<'_, AppState>) -> Result<Vec<WatchEntry>, String> {
    ...
    // One batch prices the whole watchlist (a single backend request) instead of a governed call per item.
    // We fetch fresh each poll (not the age-gated cache) to keep prices live and the history series crisp.
    let hashes: Vec<String> = items.iter().map(|i| i.market_hash_name.clone()).collect();
    let fetched = state.market.quote_batch(&hashes).await;

    let ret = retention(&state);
    let mut entries = Vec::with_capacity(items.len());
    let mut recorded = false;
    for item in items {
        let fresh = fetched.get(&item.market_hash_name);
        if let Some(q) = fresh {
            if q.lowest_price.is_some() {
                state
                    .quotes
                    .lock()
                    .unwrap()
                ...
                if let Some(cents) = q.lowest_price.as_deref().and_then(money::parse_usd_cents) {
                    ...
                    history::record(series, history::PricePoint { t_ms: q.fetched_at_ms, cents }, &ret);
                    recorded = true;
                }
            }
        }
        // Show the fresh quote, or fall back to the last-known price so a missed poll isn't shown blank.
        let quote = fresh
            .cloned()
            .or_else(|| state.quotes.lock().unwrap().get(&item.market_hash_name).cloned());
...
async fn portfolio_get(state: State<'_, AppState>) -> Result<Portfolio, String> {
    ...
    // One batch prices every holding at once (backend cache), reusing the shared quote cache.
    let hashes: Vec<String> = holdings.iter().map(|h| h.market_hash_name.clone()).collect();
    let priced = quote_many(&state, &hashes).await;

    let mut entries = Vec::with_capacity(holdings.len());
    let mut total_cents: u64 = 0;
    for holding in holdings {
        let lowest_price = priced
            .get(&holding.market_hash_name)
            .and_then(|q| q.lowest_price.clone());
...
async fn stash_advisor(
    ...
    // Price the whole stash in ONE batch: gather every candidate spelling (gear has "… A" + plain), dedup,
    // and hand them to quote_many — a single backend request instead of a per-item flood that trips our own
    // rate limit. Then resolve each job from the returned map (no network in the loop).
    let all_hashes: Vec<String> = {
        let mut v: Vec<String> = jobs.iter().flat_map(|(c, ..)| c.iter().cloned()).collect();
        v.sort_unstable();
        v.dedup();
        v
    };
    let priced = quote_many(&state, &all_hashes).await;

    let mut by_hash: HashMap<String, Agg> = HashMap::new();
    for (done, (candidates, count, category, icon, grade)) in jobs.into_iter().enumerate() {
        // The first candidate spelling that the batch could price wins; else show the item unpriced.
        let (display, price) = candidates
            .iter()
            .find_map(|h| priced.get(h))
            .map(|q| (q.market_hash_name.clone(), q.lowest_price.clone()))
            .unwrap_or_else(|| (candidates.into_iter().next().unwrap(), None));
...
/// Price many hashes at once: serve the fresh ones straight from the shared cache, then fetch every stale or
/// missing hash in ONE batch (`quote_batch` — a single backend request, or a governed per-item fan-out when
/// the backend is off). Fills the cache with priced results and, for anything still unpriced (cold/throttled),
/// falls back to its last-known cached price so nothing shows blank. This is what keeps a full stash scan to
/// one request instead of hundreds. Returns hash → quote for everything we could price.
async fn quote_many(state: &AppState, hashes: &[String]) -> HashMap<String, MarketQuote> {
    let now = gate::now_ms();
    let max_age = state.settings.quote_cache_max_age_ms;
    let mut out: HashMap<String, MarketQuote> = HashMap::with_capacity(hashes.len());
    let mut stale: Vec<String> = Vec::new();
    {
        let cache = state.quotes.lock().unwrap();
        for hash in hashes {
            match cache.get(hash) {
                Some(q) if q.lowest_price.is_some() && now.saturating_sub(q.fetched_at_ms) < max_age => {
                    out.insert(hash.clone(), q.clone());
                }
                _ => stale.push(hash.clone()),
            }
        }
    }
    stale.sort_unstable();
    stale.dedup();
    if !stale.is_empty() {
        let fetched = state.market.quote_batch(&stale).await;
        let mut cache = state.quotes.lock().unwrap();
        for (hash, quote) in fetched {
            if quote.lowest_price.is_some() {
                cache.insert(hash.clone(), quote.clone());
                out.insert(hash, quote);
            }
        }
        // Last-known fallback so a cold/throttled item shows its previous value, not a blank.
        for hash in &stale {
            if !out.contains_key(hash) {
                if let Some(q) = cache.get(hash).filter(|q| q.lowest_price.is_some()).cloned() {
                    out.insert(hash.clone(), q);
                }
            }
        }
    }
...
pub fn run() {
    tauri::Builder::default()
        ...
        // rebuilt programmatically (which came up blank). Closing the main window quits the app fully —
        // otherwise the tray icon keeps the process alive in the background and the destroyed window can't
        // be reopened from the tray.
        ...
        .setup(|app| {
            ...
            // Refresh the Sell Advisor's ItemKey→name bridge from the backend (served so a game update
            // reaches clients without a rebuild); cache it to disk and point the bridge at it. On any
            // failure or an invalid payload the bridge keeps using the map embedded at build time.
            let items_path = data_dir.join("item_map.json");
            if let Some(c) = &backend {
                if let Ok(json) = tauri::async_runtime::block_on(c.get_items()) {
                    let valid = serde_json::from_str::<serde_json::Value>(&json)
                        .map(|v| v.get("names").is_some())
                        .unwrap_or(false);
                    if valid {
                        let _ = write_atomic(&items_path, &json);
                    }
                }
            }
            save::set_map_path(items_path);
