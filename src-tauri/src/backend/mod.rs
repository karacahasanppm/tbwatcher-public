...
impl BackendClient {
    ...
    /// The Sell Advisor's ItemKey→name bridge map, as raw JSON to cache on disk verbatim (its shape is
    /// `save::mapping`'s, not this client's to parse). Backend-served so a game update reaches clients
    /// without a rebuild; the client falls back to its embedded map on any failure.
    pub async fn get_items(&self) -> Result<String, ()> {
        self.http
            .get(format!("{}/apps/tbwatcher/items", self.base))
            .send()
            .await
            .and_then(reqwest::Response::error_for_status)
            .map_err(|_| ())?
...
impl MarketDataSource for RemoteMarketSource {
    ...
    /// One backend request prices the whole set from the shared cache — no Steam, no per-item rate limit.
    /// Deliberately backend-only: cold-cache misses are left absent (the caller keeps last-known / shows
    /// "no price") rather than fanning out to Steam, which would re-create the very flood this removes. The
    /// hourly sweep fills genuine gaps; the backend cache covers the whole listed set anyway.
    async fn quote_batch(&self, hashes: &[String]) -> HashMap<String, MarketQuote> {
        let mut out = HashMap::with_capacity(hashes.len());
        if let Ok(prices) = self.client.post_prices(hashes).await {
            for (hash, price) in prices {
                if price.lowest_price.is_some() {
                    let quote = price.into_quote(&hash);
